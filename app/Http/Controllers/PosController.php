...
class PosController extends Controller
{
    public function index()
    {
        ...
        if (Auth::user()->user_type == 'admin' || Auth::user()->user_type == 'staff') {
            ...
            if (get_setting('pos_activation_for_seller') == 1) {
                ...
                $flat_discount = $shop->flat_discount;



                return view('pos.frontend.seller.pos.index', compact('customers','flat_discount'));
            }
        }
    }
    ...
    public function addToCart(Request $request)
    {
        ...
        if($stock->qty < $product->min_qty){
            return array('success' => 0, 'message' => translate("This product doesn't have enough stock for minimum purchase quantity ").$product->min_qty, 'view' => view('pos.cart',compact('flat_discount'))->render());
        }
            ...
            foreach ($request->session()->get('pos.cart') as $key => $cartItem){
                if($cartItem['id'] == $product->id && $cartItem['stock_id'] == $stock->id){
                    ...
                    if($product_stock->qty >= ($cartItem['quantity'] + 1)){
                        ...
                        return array('success' => 0, 'message' => translate("This product doesn't have more stock."), 'view' => view('pos.cart',compact('flat_discount'))->render());
                    }
                }
                $cart->push($cartItem);
            }
        ...
        return array('success' => 1, 'message' => '', 'view' => view('pos.cart',compact('flat_discount'))->render());
    }
    ...
    public function removeFromCart(Request $request)
    {
        ...
        return view('pos.cart',compact('flat_discount'));
    }
    ...
    public function get_order_summary(Request $request){
        $shop = Shop::where('user_id', Auth::user()->id)->first();
        $flat_discount = $shop->flat_discount;
        return view('pos.order_summary',compact('flat_discount'));
    }
