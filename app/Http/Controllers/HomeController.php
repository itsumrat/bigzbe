...
class HomeController extends Controller
{
    ...
    public function index()
    {
        ...
        $todays_deal_products = Cache::rememberForever('todays_deal_products', function () use ($discounted_seller_ids) {
            return filter_products(Product::whereNotIn('user_id',$discounted_seller_ids )->where('published', 1)->where('todays_deal', '1'))->get();            
        });
        $newest_products = Cache::remember('newest_products', 3600, function () use ($discounted_seller_ids) {
            ...
            return filter_products(Product::whereNotIn('user_id',$discounted_seller_ids ))->limit(12)->get();;
        });

        return view('frontend.index', compact('featured_categories', 'todays_deal_products', 'newest_products','shops'));
    }
    ...
    public function shop_schedule(Request $request)
    {
    ...
    $schedule->shop_id = $request->shop_id;
    $schedule->schedules =$request->get('d');
    $schedule->save();
    ...
    //echo $schedule->schedules['monday']['open'];

    
//return $schedule->schedules;
     // $data = json_encode($request->get('d'));
    //   $data = $request->get('d');
    //   ShopSchedule::create($data);
