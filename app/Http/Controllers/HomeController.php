...
class HomeController extends Controller
{
    ...
    {
        $get_user_value = [Auth::id(),'10203040'];
       $user_id = '';
      
        if(Auth::user())
        {
             $top_products =  Product::withCount([
                'views' => function ($query) use($get_user_value) {
                    $query->where('date','>=',Carbon::now()->subDays(30)->toDateString())
                    ->where('date','<=',Carbon::now()->toDateString())
                    ->whereIn('user_id',$get_user_value);
                }
            ])->withCount([
        ...
        $discounted_seller_ids = Shop::where('is_discounted',1)->pluck('user_id');
        ...
        // });
        $todays_deal_products = Cache::rememberForever('todays_deal_products', function () use ($discounted_seller_ids){
            return filter_products(Product::where('published', 1)->whereNotIn('user_id',$discounted_seller_ids )->where('todays_deal', '1'))->get();
        });

        $newest_products = Cache::remember('newest_products', 3600, function () use ($discounted_seller_ids) {
            return filter_products(Product::whereNotIn('user_id',$discounted_seller_ids )->latest())->limit(12)->get();
        });
        return view('frontend.index', compact('featured_categories', 'todays_deal_products', 'newest_products','shops', 'top_products', 'user_id'));
    }
    ...
    public function load_best_discounted_sellers_section(){
        return view('frontend.partials.best_discounted_sellers_section');
    }
...
//Imran
    public function shop($slug)
    {
        ...
        if ($shop != null) {
            if ($shop->verification_status != 0) {
                    ...
                    return view('frontend.bigzloot.single', compact('shop'));
    ...
    {
        $shops = Shop::whereIn('user_id', verified_sellers_id())->where('is_discounted',null)
            ->paginate(15);
