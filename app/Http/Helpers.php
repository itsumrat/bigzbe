...
if (!function_exists('filter_products')) {
    function filter_products($products)
    {
       ...
       $discounted_seller = Shop::where('is_discounted',1)->pluck('user_id');

        $verified_sellers = verified_sellers_id();
        if (get_setting('vendor_system_activation') == 1) {
            return $products->where('approved', '1')->where('published', '1')->whereNotIn('user_id',$discounted_seller)->where('auction_product', 0)->orderBy('created_at', 'desc')->where(function ($p) use ($verified_sellers) {
...
//bigzloot discount_in_percentage
if (!function_exists('flat_discount_in_percentage')) {
    function flat_discount_in_percentage($product)
    {
        $base = home_base_price($product, false);
        $reduced = flat_discounted_base_price($product, false);
        $discount = $base - $reduced;
        $dp = ($discount * 100) / ($base > 0 ? $base : 1);
        return round($dp);
        dd($dp);
...
//Shows Base Price with Flat discount
if (!function_exists('flat_discounted_base_price')) {
    function flat_discounted_base_price($product, $formatted = true)
    {
        $flat_discount = \App\Models\Shop::where('user_id',$product->user_id)->value('flat_discount');
        ...
        if($flat_discount!=null){
            $price -= ($price * $flat_discount) / 100;
        }
